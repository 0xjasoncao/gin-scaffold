...
func RateLimitMiddleware(redisClient redis.UniversalClient, conf *RateLimiterConfig) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		tokenInfo := ginutil.TokenFromContext(ctx)
		...
		for i, rule := range conf.Rules {
			if rule.Regex != nil && rule.Regex.Match([]byte(requestURL)) {
				...
				if err != nil {
					...
					ginutil.ResError(c, errorsx.NewInternal("Internal server error, please try again later."))
					return
				}
				if !allowed {
					...
					ginutil.ResError(c, errorsx.NewTooManyRequest("Too many requests, please try again later."))
					return
				}
			}
