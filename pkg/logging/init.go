...
func InitLogger(cfg *config.Config) error {
	if cfg == nil || len(cfg.Logger.Outputs) == 0 {
	...
	for name, output := range cfg.Logger.Outputs {
		if !output.Enabled {
			continue
		}
		// Create log writer
		...
		if err != nil {
			return errors.Wrapf(err, "Failed to create output target [%s]", name)
		}
...
// 创建日志编码器
func createEncoder(output config.Output) zapcore.Encoder {
...
// 创建日志写入器
func createWriteSyncer(output config.Output) (zapcore.WriteSyncer, error) {
